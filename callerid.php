...
function __autoload($class_name) {
    if(endswith($class_name,'NumberCleaner') && $class_name!='NumberCleaner'){
        require_once('number_cleaners/' . strtolower(substr($class_name,0,-13)) .'.php');
    }else if(endswith($class_name,'Source')){
...
if(empty($thenumber_orig)){
    ...
    $config['number_cleaners'] = array(
        'au', 'ch', 'nanp', 'se', 'pt', 'be', 'fr', 'lu', 'dk', 'at', 'it', 'de'
    );
